...
newWorld <- function(params=hyper3params()){
    ...
    within(params,{ 
        ...
        for (i in l){
        	...
        	#print(cell)
        	#print(ruleList[i,"inBall"])
        	#print(ruleList[i,"outBall"])
        }
...
iterWorld <- function(world,niter=1){
    ...
    within(world,{
        ...
        while(insidetick <= niter){ ## do all of this niter times
            ...
            ############
         	#print("random int:")
            #print(sample.int(100,size=1))	            
            g <- c()
            cnts <- c()           
            for (i in seq.int(nrow(ruleInv))){
            	count <- sum(ruleInv[i,])
            	cnts <- c(cnts,count)
            	if (count != 0){
            		g <- c(g,rep(i,count))
            ...
            g <- c()
            cnts <- c() 
            for (i in seq.int(length(ruleInv[thisBin,]))){
            	if (ruleInv[thisBin,i] != 0){
            		count <- ruleInv[thisBin,i]
            		cnts <- c(cnts,count)
            		g <- c(g,rep(i,count))
            		}
            	}
            ...
            #print(thisBin)	
            ##print(g)
            if (length(g) != 1){thisRule <- sample(g,1)}
            else{thisRule <- g
            	a <- sample(g,1)}	
            
            #print(ruleList[thisRule,"inBall"])
            #print(ruleList[thisRule,"outBall"])
            ...
            g <- c()
            for (i in seq.int(length(ballEnv))){g <- c(g,rep(i,ballEnv[i]))}
            ...
            if(
                ...
                (
                    ...
                    sample(g,1) == ruleList[thisRule,"inBall"]
                )
            ){
            ...
            while(!is.null(curBall)){
                ...
                if(!any(am[thisBin,]>0)){ ## thisBin has no neighbors
					...
					nghbs = c()
					##print(length(am[thisBin,]))
					for (i in seq.int(length(am[thisBin,]))){
						if (am[thisBin,i] == 1){
							nghbs <- c(nghbs, i)
						}
					}
                    ...
                    if(!is.null(curBall) && thisBin != nextBin){
						...
						#print("successful pass to")
						#print(nextBin)
						#print(ruleInv[nextBin,])
                        g <- c()
                        for (i in compatRules){
                        	c <- ruleInv[nextBin,i]
                        	g <- c(g,rep(i,c))
                        }
                        #print(g)
                        ...
                        if(reproduceTarget){ ## target or reciprocal reproduction
                            ...
                            #print("reproduced rule below")
                            #print(ruleList[nextRule,"inBall"])
            				#print(ruleList[nextRule,"outBall"])
            			...
            			#print(g)
            			dBin <- sample(g,1)
            			#print("drew agent for deletion..")
                        #print(dBin)
            			g <- c()
            			for (i in seq.int(length(ruleInv[dBin,]))){
            			    if (ruleInv[dBin,i] != 0){
            					count <- ruleInv[dBin,i]
            					g <- c(g,rep(i,count))
            				}
            			}
						#print(g)
                        ...
                        #print("Deleting a rule ")
                        #print(dRule)
                        ruleInv[dBin,dRule] <- ruleInv[dBin,dRule] - 1
                        ############
